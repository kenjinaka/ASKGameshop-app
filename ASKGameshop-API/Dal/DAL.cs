...
namespace ASKGames.Dal
{
    public class DAL : IDAL
    {
        ...
        public bool VerificarClienteContemSacola(Cliente cliente)
        {
            ...
            && x.Status == ASKGameShopAPI.Models.Enum.StatusPedido.Sacola).FirstOrDefault();
        ...
        {
            return dbContext.Pedido.Include(x=> x.FormaPagamento).Include(x=> x.FormaPagamento.Cartao).Where(x => x.Cliente.Id == cliente.Id && x.Status == ASKGameShopAPI.Models.Enum.StatusPedido.PedidoFinalizado).ToList();
        }
